...
const Sidebar = ({collapsed}) => {
    ...
    const parkingLotName = CredentialStore.parkingLot
        ? (CredentialStore.parkingLot.length > 16
            ? CredentialStore.parkingLot.slice(0, 15) + '...'
            : CredentialStore.parkingLot)
        : 'LX PARKING';
    const parkingLotFullName = CredentialStore.parkingLot || 'LX PARKING';
        ...
        <div className={`sidebar-container${collapsed ? ' collapsed' : ''}`}>
            ...
            <div className="parking-lot-name">
                {!collapsed ? (
                    ...
                    <div className="parking-lot-name-short">{getInitials(parkingLotFullName)}</div>
                )}
            </div>
            ...
            <nav className="sidebar-nav">
                {sidebarOptions.map(({to, icon: Icon, label, circle, exact}) => {
                    ...
                    return (
                        ...
                        >
              <span
                  className={`sidebar-icon-wrapper${circle ? ' sidebar-icon-circle' : ''}${isActive ? ' active-icon' : ''}`}>
