    ...
    useEffect(() => {
        const fetchSubscriptions = async () => {
            try {
                ...
                const response = await fetch('/parkinglot/api/v1/fetch/subscription', {
                    method: 'POST',
                    headers: {
                ...
                if (data.subscriptionUserInfoList) {
                    MOCK_SUBSCRIPTIONS.length = 0; // Clear existing mock data
                    MOCK_SUBSCRIPTIONS.push(...data.subscriptionUserInfoList.map(sub => ({
                        subscriptionId: sub.subscriptionId,
                        vehicleNumber: sub.vehicleNumber,
                        subscriptionType: sub.subscriptionType,
                        vehicleType: 'N/A', // Default value, adjust as needed
                        employeeName: 'N/A', // Placeholder, adjust as needed
                        employeeId: 'N/A', // Placeholder, adjust as needed
                        startDate: sub.purchasedOn,
                        endDate: sub.validTill,
                        plan: sub.subscriptionFrequency,
                        amount: sub.amountPaid,
                        status: sub.status,
                        autoRenew: false, // Default value, adjust as needed
                        parkingLotId: 'N/A', // Placeholder, adjust as needed
                        spotNumber: 'N/A' // Placeholder, adjust as needed
                    })));
                }
            } catch (error) {
